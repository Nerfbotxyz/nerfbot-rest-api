...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.withSchema(SCHEMA_NAME).createTable('exports', table => {
    table.uuid('exportId')
      .unique()
      .notNullable()
      .primary()
  ...
  await knex.schema.withSchema(SCHEMA_NAME).raw(onUpdateTrigger('exports'))
}

export async function down(knex: Knex): Promise<void> {
  await knex.schema.withSchema(SCHEMA_NAME).dropTableIfExists('exports')
}
